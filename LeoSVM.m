...
% 整体 放入 S V M Matlab 训练器 中训练得到模型
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%以下为程序控制部分     你要设置的
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for flower_index=1:Flower_Num
    ...
    for i = 1:Train_Set_Num
        ...
        IMG_HOG_F(:,i,flower_index)=imh_hog_featureVec';
    end
...
for flower_index=1:Flower_Num
    ...
    result_Rate_T=length(find(SVM_Result==1))/(length(SVM_Result)); % 计算 每个类别测试的识别正确率
    correct=correct+length(find(SVM_Result==1));
    if (Flag_Use_Random_Index==1)
        disp(['第',num2str(Random_Index(flower_index)),'类花的测试图片有 ',num2str(Test_Pic_Num),' 张SVM方法识别率为 ',num2str(result_Rate_T*100),' %'])
    else
        disp(['第',num2str(flower_index),'类花的测试图片有 ',num2str(Test_Pic_Num),' 张SVM方法识别率为 ',num2str(result_Rate_T*100),' %'])
    end
    if(Flag_Test_Detail==1) % 判断是否计算测试细节
        disp(['细节：',num2str(SVM_Result)])
    end
end
disp('S V M 测试完成 ')
disp(' ')
...
disp(['S V M 算法 训练 ',num2str(Flower_Num),' 种花的分类情况，总识别率为: ',num2str(result_Rate*100),' %'])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
