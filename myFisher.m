...
Files = dir(fullfile('D:\毕设\程序\train\','*.jpg'));
...
for i = 1:LengthFiles/3
   img = imread(strcat('D:\毕设\程序\train\',Files(i).name));%读取图像
...
for i = LengthFiles/3+1:2*LengthFiles/3
    img =imread(strcat('D:\毕设\程序\train\',Files(i).name));
    img1 = rgb2gray(img);
    if i>=1 && i<=5
    figure;imshow(img1)
    end
...
%以下为测试部分
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Files = dir(fullfile('D:\毕设\程序\test\','*.jpg'));
LengthFiles = length(Files);
correct=0;
for i=1:30
    img = imread(strcat('D:\毕设\程序\test\',Files(i).name));
    img1 = rgb2gray(img);
...
%计算识别率
result = correct/LengthFiles*3.5;
disp(['Fisher方法识别率为 ',num2str(result)])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
